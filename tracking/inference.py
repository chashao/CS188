...
class ExactInference(InferenceModule):
    ...
    def elapseTime(self, gameState):
        ...
        *** GORY DETAIL AHEAD ***
        ...
        allPossible = util.Counter()
        for p in self.legalPositions:
            newPositions = self.getPositionDistribution(self.setGhostPosition(gameState, p))
            for np, probability in newPositions.items():
                allPossible[np] += self.beliefs[p] * probability
...
class ParticleFilter(InferenceModule):
    ...
    def observe(self, observation, gameState):
        ...
        if noisyDistance != None:
            ...
            for particle in self.particles:
                ...
                allPossible[particle] += emissionModel[distance]
            if allPossible.totalCount() == 0:
                self.initializeUniformly(gameState)
            else:
                lst = list()
                for i in range(0, self.numParticles):
                    lst.append(util.sample(allPossible))
                self.particles = lst
        else:
    ...
    def elapseTime(self, gameState):
        ...
        for p in self.particles:
            newPositions = self.getPositionDistribution(self.setGhostPosition(gameState, p))
            newParticles.append(util.sample(newPositions))
...
class JointParticleFilter:
    ...
    def initializeParticles(self):
        ...
        "*** YOUR CODE HERE ***"
        self.particles = list()
        ghostTuples = list(itertools.product(self.legalPositions, repeat = self.numGhosts))
        for i in range(0, self.numParticles):
            self.particles.append(ghostTuples[i % len(ghostTuples)])
        ...
        for particle, prob in beliefs.items():
            ...
            for ghost in range(0, self.numGhosts):
                if noisyDistances[ghost] != None:
                    allPossible[particle] *= emissionModels[ghost][util.manhattanDistance(particle[ghost], pacmanPosition)]
        beliefs = allPossible
        beliefs.normalize()
    ...
    def elapseTime(self, gameState):
        ...
        for oldParticle in self.particles:
            ...
            "*** YOUR CODE HERE ***"
            ghostPositions = setGhostPositions(gameState, newParticle)
            for n in range(0, self.numGhosts):
                newPositions = getPositionDistributionForGhost(ghostPositions, n, self.ghostAgents[n])
                newParticle[n] = util.sample(newPositions)
            "*** END YOUR CODE HERE ***"
