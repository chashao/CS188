...
def inferenceByVariableEliminationWithCallTracking(callTrackingList=None):
    ...
    def inferenceByVariableElimination(bayesNet, queryVariables, evidenceDict, eliminationOrder):
        ...
        eliminationVariables = (bayesNet.variablesSet() - evidenceVariablesSet) - queryVariablesSet
        currentFactorsList = bayesNet.getAllCPTsWithEvidence(evidenceDict)
        for currentVar in eliminationOrder:
            currentFactorsList, joinedFactor = joinFactorsByVariable(currentFactorsList, currentVar)
            unconditionedVariables = joinedFactor.unconditionedVariables()
            if currentVar in unconditionedVariables and len(unconditionedVariables) >1:
                joinedFactor = eliminate(joinedFactor, currentVar)
                currentFactorsList.append(joinedFactor)
        factor = joinFactors(currentFactorsList)
...
def inferenceByLikelihoodWeightingSamplingRandomSource(randomSource=None):
    ...
    def inferenceByLikelihoodWeightingSampling(bayesNet, queryVariables, evidenceDict, numSamples):
        ...
        "*** YOUR CODE HERE ***"
        reducedDomain = bayesNet.getReducedVariableDomains(evidenceDict)
        newFactor = Factor(queryVariables, evidenceDict, reducedDomain)
        for i in range(0, numSamples):
            ...
            for variable in bayesNet.linearizeVariables():
                ...
                if variable in evidenceDict:
                    currSample[variable] = bayesNet.getReducedVariableDomains(evidenceDict)[variable][0]
                    w *= factor.getProbability(currSample);
                else:
                    currSample.update(sampleFromFactor(factor, currSample))
            oldProb = newFactor.getProbability(currSample)
            newFactor.setProbability(currSample, oldProb+w)
